...
const NutrientRow = ({ label, value, goal, unit, color, t }: any) => {

  const safeGoal = goal || 1;
  const percentage = Math.min((value / safeGoal) * 100, 100);
  ...
  const handleWatchAd = async () => {
    ...
    try {
      ...
      if (rewarded) {
        ...
        const unlockUntil = Date.now() + 3600000;
              ...
              >
                ...
                <div className="flex items-center justify-center gap-2">
                  ...
                  <p className="text-sm font-bold text-foreground">
                    {loadingAd ? "Carregando anúncio..." : t('detail_watchAdMacros')}
                  </p>

                </div>

                <p className="text-xs text-muted-foreground">
                  {t('detail_watchAdDesc')}
                </p>
